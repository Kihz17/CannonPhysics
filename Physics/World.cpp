...
void World::IntegrateParticles(float deltaTime)
{
	...
	itParticle = this->particles.begin();
	while (itParticle != this->particles.end())
	{
		std::vector<Particle*>::iterator itTarget = this->targets.begin();
		while (itTarget != this->targets.end())
		{
			Particle* particle = *itParticle;
			Particle* target = *itTarget;
			if (collisionDetector.CheckCollision(particle, target))
			{
				particle->OnCollision(target);
				target->OnCollision(particle);
			}
			itTarget++;
		}
